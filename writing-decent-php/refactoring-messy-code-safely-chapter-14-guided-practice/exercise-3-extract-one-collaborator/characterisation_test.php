...
/**
 * Same five-case characterisation test, untouched. After extracting
 * `InvoiceTotalsCalculator` it must still pass byte-for-byte.
 */
...
function run_characterisation_test(): void
{
    ...
    if ($failures !== []) {
        echo "\nCharacterisation test FAILED after extract — the seam moved the behaviour, that is a bug.\n" . implode("\n", $failures) . "\n";
        exit(1);
    }
    echo "characterisation test: PASS (5/5 cases pinned, post-extract)\n";
}
